...
describe('convert', () => {
  ...
  describe('oneToMany.atomicToUnix', () => {
    ...
    it('start of 1972', () => {
      expect(atomicToUnix(Date.UTC(1972, JAN, 1, 0, 0, 9, 998))).toBe(Date.UTC(1972, JAN, 1, 0, 0, 0, 105))
      expect(atomicToUnix(Date.UTC(1972, JAN, 1, 0, 0, 9, 998))).toBe(63072000105) // same

      expect(atomicToUnix(Date.UTC(1972, JAN, 1, 0, 0, 9, 999))).toBe(Date.UTC(1972, JAN, 1, 0, 0, 0, 106))
      expect(atomicToUnix(Date.UTC(1972, JAN, 1, 0, 0, 9, 999))).toBe(63072000106) // same

      // After this point in time, conversions become far simpler and always integer numbers of milliseconds
      expect(atomicToUnix(Date.UTC(1972, JAN, 1, 0, 0, 10, 0))).toBe(Date.UTC(1972, JAN, 1, 0, 0, 0, 0))
      expect(atomicToUnix(Date.UTC(1972, JAN, 1, 0, 0, 10, 1))).toBe(Date.UTC(1972, JAN, 1, 0, 0, 0, 1))
      expect(atomicToUnix(Date.UTC(1972, JAN, 1, 0, 0, 10, 2))).toBe(Date.UTC(1972, JAN, 1, 0, 0, 0, 2))
      // etc.
    })

    it('typical', () => {
      // A typical leap second from the past, note repetition
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 29, 750))).toBe(Date.UTC(1998, DEC, 31, 23, 59, 58, 750))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 30, 0))).toBe(Date.UTC(1998, DEC, 31, 23, 59, 59, 0))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 30, 250))).toBe(Date.UTC(1998, DEC, 31, 23, 59, 59, 250))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 30, 500))).toBe(Date.UTC(1998, DEC, 31, 23, 59, 59, 500))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 30, 750))).toBe(Date.UTC(1998, DEC, 31, 23, 59, 59, 750))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 31, 0))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 0, 0))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 31, 250))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 0, 250))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 31, 500))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 0, 500))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 31, 750))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 0, 750))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 32, 0))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 0, 0)) // repetition
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 32, 250))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 0, 250)) // repetition
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 32, 500))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 0, 500)) // repetition
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 32, 750))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 0, 750)) // repetition
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 33, 0))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 1, 0))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 33, 250))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 1, 250))
    })
  })
  ...
  describe('oneToOne.atomicToUnix', () => {
    ...
    it('0.107758 seconds added, start of 1972', () => {
      expect(atomicToUnix(Date.UTC(1972, JAN, 1, 0, 0, 9, 890))).toBe(Date.UTC(1971, DEC, 31, 23, 59, 59, 997))
      expect(atomicToUnix(Date.UTC(1972, JAN, 1, 0, 0, 9, 891))).toBe(Date.UTC(1971, DEC, 31, 23, 59, 59, 998))
      expect(atomicToUnix(Date.UTC(1972, JAN, 1, 0, 0, 9, 892))).toBe(Date.UTC(1971, DEC, 31, 23, 59, 59, 999))
    ...
    it('typical', () => {
      ...
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 32, 0))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 0, 0))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 32, 250))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 0, 250))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 32, 500))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 0, 500))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 32, 750))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 0, 750))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 33, 0))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 1, 0))
      expect(atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 33, 250))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 1, 250))
    })
  })
  ...
  describe('Unix->TAI conversions', () => {
    ...
    it('icky', () => {
      ...
      expect(taiConverter.oneToMany.unixToAtomic(Date.UTC(1972, JAN, 1, 0, 0, 0, 1)).length).toBe(2)
      expect(taiConverter.oneToMany.unixToAtomic(Date.UTC(1972, JAN, 1, 0, 0, 0, 1))[1]).toBe(Date.UTC(1972, JAN, 1, 0, 0, 10, 1))
      expect(taiConverter.oneToOne.unixToAtomic(Date.UTC(1972, JAN, 1, 0, 0, 0, 2))).toBe(Date.UTC(1972, JAN, 1, 0, 0, 10, 2))
      // etc.
    })

    it('typical', () => {
      // A typical leap second from the past, note repetition
      expect(taiConverter.oneToOne.unixToAtomic(Date.UTC(1998, DEC, 31, 23, 59, 58, 750))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 29, 750))
      expect(taiConverter.oneToOne.unixToAtomic(Date.UTC(1998, DEC, 31, 23, 59, 59, 0))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 30, 0))
      expect(taiConverter.oneToOne.unixToAtomic(Date.UTC(1998, DEC, 31, 23, 59, 59, 250))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 30, 250))
      expect(taiConverter.oneToOne.unixToAtomic(Date.UTC(1998, DEC, 31, 23, 59, 59, 500))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 30, 500))

      expect(taiConverter.oneToMany.unixToAtomic(Date.UTC(1998, DEC, 31, 23, 59, 59, 750))).toEqual([
        Date.UTC(1999, JAN, 1, 0, 0, 30, 750)
      ])
      expect(taiConverter.oneToOne.unixToAtomic(Date.UTC(1998, DEC, 31, 23, 59, 59, 750))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 30, 750))
      ...
      expect(taiConverter.oneToOne.unixToAtomic(Date.UTC(1999, JAN, 1, 0, 0, 0, 0))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 32, 0))
      ...
      expect(taiConverter.oneToOne.unixToAtomic(Date.UTC(1999, JAN, 1, 0, 0, 0, 250))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 32, 250))
      ...
      expect(taiConverter.oneToOne.unixToAtomic(Date.UTC(1999, JAN, 1, 0, 0, 0, 500))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 32, 500))
      ...
      expect(taiConverter.oneToOne.unixToAtomic(Date.UTC(1999, JAN, 1, 0, 0, 0, 750))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 32, 750))

      expect(taiConverter.oneToMany.unixToAtomic(Date.UTC(1999, JAN, 1, 0, 0, 1, 0))).toEqual([
        Date.UTC(1999, JAN, 1, 0, 0, 33, 0)
      ])
      expect(taiConverter.oneToOne.unixToAtomic(Date.UTC(1999, JAN, 1, 0, 0, 1, 0))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 33, 0))
      expect(taiConverter.oneToOne.unixToAtomic(Date.UTC(1999, JAN, 1, 0, 0, 1, 250))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 33, 250))
    })

    it('Now-ish', () => {
      expect(taiConverter.oneToOne.unixToAtomic(Date.UTC(2016, OCT, 27, 20, 5, 14, 678))).toBe(Date.UTC(2016, OCT, 27, 20, 5, 50, 678))
    })
  })

  it('Demo', () => {
    expect(taiConverter.oneToOne.unixToAtomic(915148799000)).toBe(915148830000)
    expect(taiConverter.oneToOne.unixToAtomic(Date.UTC(1998, DEC, 31, 23, 59, 59))).toBe(Date.UTC(1999, JAN, 1, 0, 0, 30))
    expect(taiConverter.oneToOne.atomicToUnix(915148830000)).toBe(915148799000)
    expect(taiConverter.oneToOne.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 30))).toBe(Date.UTC(1998, DEC, 31, 23, 59, 59))
    expect(taiConverter.oneToMany.unixToAtomic(915148800000)).toEqual([915148831000, 915148832000])
    expect(taiConverter.oneToMany.atomicToUnix(915148831000)).toBe(915148800000)
    expect(taiConverter.oneToMany.atomicToUnix(915148832000)).toBe(915148800000)
  ...
  it('inserted tenth', () => {
    ...
    expect(taiConverter.oneToMany.unixToAtomic(Date.UTC(1965, SEP, 1, 0, 0, 0, 50)).length).toBe(2)
    expect(Math.abs(taiConverter.oneToMany.unixToAtomic(Date.UTC(1965, SEP, 1, 0, 0, 0, 50))[0] - (Date.UTC(1965, SEP, 1, 0, 0, 4, 105) + 0.058)) < 0.0001)
    expect(Math.abs(taiConverter.oneToMany.unixToAtomic(Date.UTC(1965, SEP, 1, 0, 0, 0, 50))[1] - (Date.UTC(1965, SEP, 1, 0, 0, 4, 205) + 0.058)) < 0.0001)
  })
