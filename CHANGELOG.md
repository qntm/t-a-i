...
Rather than `require('t-a-i')` returning a converter object, it now returns an object `tai` whose property `tai.Converter` must be called to create a converter object. It also returns constants `INSERT_OVERRUN_ARRAY` and `INSERT_STALL_LAST` which must be used to select the conversion model.

In general, `tai.oneToMany` is replacesd with `Converter(INSERT_OVERRUN_ARRAY)`, and `tai.oneToOne` is replaced with `Converter(INSERT_STALL_LAST)` replaces `tai.oneToOne`.
