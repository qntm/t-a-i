  ...
  describe('OVERRUN', () => {
    ...
    describe('unixToAtomic (array mode)', () => {
      ...
      it('starts TAI at 1961-01-01 00:00:01.422_818', () => {
        assert.deepEqual(
          unixToAtomic(Date.UTC(1961, JAN, 1, 0, 0, 0, 0) * 1_000_000),
      ...
      it('advances 15 nanoseconds per second', () => {
        assert.deepEqual(
          unixToAtomic(Date.UTC(1961, JAN, 1, 0, 0, 1, 0) * 1_000_000),
          [-283_996_797_577_181_980] // inaccurate
        )
        assert.deepEqual(
          unixToAtomic(BigInt(Date.UTC(1961, JAN, 1, 0, 0, 1, 0)) * 1_000_000n),
      ...
      it('advances 0.001_296 TAI seconds per Unix day', () => {
        assert.deepEqual(
          unixToAtomic(Date.UTC(1961, JAN, 2, 0, 0, 0, 0) * 1_000_000),
          [Date.UTC(1961, JAN, 2, 0, 0, 1, 424) * 1_000_000 + 114_000]
        )
        assert.deepEqual(
          unixToAtomic(BigInt(Date.UTC(1961, JAN, 2, 0, 0, 0, 0)) * 1_000_000n),
    ...
    describe('atomicToUnix', () => {
      ...
      it('typical', () => {
        ...
        assert.equal(taiConverter.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 30, 0) * 1_000_000), Date.UTC(1998, DEC, 31, 23, 59, 59, 0) * 1_000_000)
        assert.equal(taiConverter.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 30, 250) * 1_000_000), Date.UTC(1998, DEC, 31, 23, 59, 59, 250) * 1_000_000)
        assert.equal(taiConverter.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 30, 500) * 1_000_000), Date.UTC(1998, DEC, 31, 23, 59, 59, 500) * 1_000_000)
        assert.equal(taiConverter.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 30, 750) * 1_000_000), Date.UTC(1998, DEC, 31, 23, 59, 59, 750) * 1_000_000)
        assert.equal(taiConverter.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 31, 0) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 0, 0) * 1_000_000)
        assert.equal(taiConverter.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 31, 250) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 0, 250) * 1_000_000)
        assert.equal(taiConverter.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 31, 500) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 0, 500) * 1_000_000)
        assert.equal(taiConverter.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 31, 750) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 0, 750) * 1_000_000)
        assert.equal(taiConverter.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 32, 0) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 0, 0) * 1_000_000) // repetition
        assert.equal(taiConverter.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 32, 250) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 0, 250) * 1_000_000) // repetition
        assert.equal(taiConverter.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 32, 500) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 0, 500) * 1_000_000) // repetition
        assert.equal(taiConverter.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 32, 750) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 0, 750) * 1_000_000) // repetition
        assert.equal(taiConverter.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 33, 0) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 1, 0) * 1_000_000)
        assert.equal(taiConverter.atomicToUnix(Date.UTC(1999, JAN, 1, 0, 0, 33, 250) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 1, 250) * 1_000_000)
      })
    })
    ...
    describe('atomicToOffset', () => {
      it('typical', () => {
        // A typical leap second from the past, note repetition
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 29, 750)) * 1_000_000n), 31_000_000_000n)
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 30, 0)) * 1_000_000n), 31_000_000_000n)
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 30, 250)) * 1_000_000n), 31_000_000_000n)
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 30, 500)) * 1_000_000n), 31_000_000_000n)
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 30, 750)) * 1_000_000n), 31_000_000_000n)
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 31, 0)) * 1_000_000n), 31_000_000_000n)
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 31, 250)) * 1_000_000n), 31_000_000_000n)
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 31, 500)) * 1_000_000n), 31_000_000_000n)
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 31, 750)) * 1_000_000n), 31_000_000_000n)
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 32, 0)) * 1_000_000n), 32_000_000_000n)
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 32, 250)) * 1_000_000n), 32_000_000_000n)
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 32, 500)) * 1_000_000n), 32_000_000_000n)
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 32, 750)) * 1_000_000n), 32_000_000_000n)
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 33, 0)) * 1_000_000n), 32_000_000_000n)
        assert.equal(taiConverter.atomicToOffset(BigInt(Date.UTC(1999, JAN, 1, 0, 0, 33, 250)) * 1_000_000n), 32_000_000_000n)
      })
    })
    ...
    describe('TAI->Unix conversions', () => {
      it('now-ish', () => {
        assert.equal(
          taiConverter.atomicToUnix(BigInt(Date.UTC(2016, OCT, 27, 20, 5, 50, 678)) * 1_000_000n),
          BigInt(Date.UTC(2016, OCT, 27, 20, 5, 14, 678)) * 1_000_000n
        )
        assert.equal(
          taiConverter.atomicToOffset(BigInt(Date.UTC(2016, OCT, 27, 20, 5, 50, 678)) * 1_000_000n),
          36_000_000_000n
        )
      })
    })
    ...
    describe('Unix->TAI conversions', () => {
      it('The NEW earliest instant in TAI', () => {
        assert.deepEqual(
          taiConverter.unixToAtomic(Date.UTC(1960, DEC, 31, 23, 59, 59, 999) * 1_000_000, { array: true }),
          []
        )
        assert.equal(
          taiConverter.unixToAtomic(Date.UTC(1960, DEC, 31, 23, 59, 59, 999) * 1_000_000),
          NaN
        )
      })
      ...
      it('icky', () => {
        ...
        assert.deepEqual(
          taiConverter.unixToAtomic(Date.UTC(1972, JAN, 1, 0, 0, 0, 0) * 1_000_000, { array: true }),
          [
        ...
        assert.equal(
          taiConverter.unixToAtomic(Date.UTC(1972, JAN, 1, 0, 0, 0, 0) * 1_000_000),
          Date.UTC(1972, JAN, 1, 0, 0, 10, 0) * 1_000_000
        )
        assert.equal(
          taiConverter.unixToAtomic(Date.UTC(1972, JAN, 1, 0, 0, 0, 1) * 1_000_000, { array: true }).length,
          2
        )
        assert.equal(
          taiConverter.unixToAtomic(Date.UTC(1972, JAN, 1, 0, 0, 0, 1) * 1_000_000, { array: true })[1],
          Date.UTC(1972, JAN, 1, 0, 0, 10, 1) * 1_000_000
        )
        assert.equal(
          taiConverter.unixToAtomic(Date.UTC(1972, JAN, 1, 0, 0, 0, 1) * 1_000_000),
          Date.UTC(1972, JAN, 1, 0, 0, 10, 1) * 1_000_000
        )
        assert.equal(
          taiConverter.unixToAtomic(Date.UTC(1972, JAN, 1, 0, 0, 0, 2) * 1_000_000),
      ...
      it('typical', () => {
        // A typical leap second from the past, note repetition
        assert.equal(taiConverter.unixToAtomic(Date.UTC(1998, DEC, 31, 23, 59, 59, 750) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 30, 750) * 1_000_000)
        assert.equal(taiConverter.unixToAtomic(Date.UTC(1999, JAN, 1, 0, 0, 0, 0) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 32, 0) * 1_000_000)
        assert.equal(taiConverter.unixToAtomic(Date.UTC(1999, JAN, 1, 0, 0, 0, 250) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 32, 250) * 1_000_000)
        assert.equal(taiConverter.unixToAtomic(Date.UTC(1999, JAN, 1, 0, 0, 0, 500) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 32, 500) * 1_000_000)
        assert.equal(taiConverter.unixToAtomic(Date.UTC(1999, JAN, 1, 0, 0, 0, 750) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 32, 750) * 1_000_000)
        assert.equal(taiConverter.unixToAtomic(Date.UTC(1999, JAN, 1, 0, 0, 1, 0) * 1_000_000), Date.UTC(1999, JAN, 1, 0, 0, 33, 0) * 1_000_000)
      })
    })
  })
